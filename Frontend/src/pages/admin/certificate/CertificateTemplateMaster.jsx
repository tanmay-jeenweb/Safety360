...
// ─── Modal Form Component for Add & Edit Certificate Template ────────────────
function CertificateTemplateFormModal({ isOpen, onClose, onSave, editingRow, saving }) {
    const [formatPrefix, setFormatPrefix] = useState("");

    useEffect(() => {
        if (editingRow) {
            setFormatPrefix(editingRow.format_prefix || "");
        } else {
            setFormatPrefix("");
        }
    }, [editingRow, isOpen]);
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (!formatPrefix.trim()) {
            toast.error("Formate prefix is required");
            return;
        }
        onSave(formatPrefix.trim());
    };
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    <div>
                        <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: "#fff" }}>
                            {editingRow ? "Edit Certificate Template" : "Add Certificate Template"}
                        </h2>
                        <p style={{ margin: "4px 0 0", fontSize: 13, color: "#d9e2ec" }}>
                            {editingRow ? "Update certificate template details below" : "Fill in the details to add a new certificate template"}
                        </p>
                    </div>
                    <button
                        type="button"
                ...
                <form onSubmit={handleSubmit}>
                    <div style={{ padding: "24px" }}>
                        <div style={{ marginBottom: 16 }}>
                            <label style={{
                                ...
                                Formate Prefix <span style={{ color: "#e11d48" }}>*</span>
                            </label>
                            <input
                                type="text"
                                value={formatPrefix}
                                onChange={(e) => setFormatPrefix(e.target.value)}
                                placeholder="Enter formate prefix..."
                                style={{
                    ...
                    <div style={{
                        ...
                        <button
                            type="submit"
                            disabled={saving || !formatPrefix.trim()}
                            style={{
                        ...
                        >
                            {saving ? "Saving…" : editingRow ? "Save Changes" : "Create Template"}
                        </button>
                    </div>
                </form>
            </div>
...
// ─── Main Certificate Template Master Page Component ────────────────────────────
export default function CertificateTemplateMaster() {
    ...
    const loadTemplates = async () => {
        ...
        try {
            ...
            setError("Unable to load certificate templates. Please try again.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSave = async (formatPrefix) => {
        setSaving(true);
        try {
            if (editingRow) {
                await updateCertificateTemplate(editingRow.id, { formatPrefix });
                toast.success("Certificate template updated successfully");
            } else {
                await createCertificateTemplate({ formatPrefix });
            ...
            toast.error(err?.response?.data?.message || "Unable to save certificate template.");
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            toast.error(err?.response?.data?.message || "Unable to delete certificate template.");
        } finally {
            setSaving(false);
        }
    };
    ...
    const columns = useMemo(() => {
        const cols = [
            ...
            {
                key: "format_prefix",
                label: "Formate Prefix",
                render: (row) => <span style={{ fontWeight: 700, color: "#1e293b" }}>{row.format_prefix}</span>
            }
        ];

        const canUpdate = hasPermission("certificate_template_master", "update");
        const canDelete = hasPermission("certificate_template_master", "delete");
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: "100vh", background: "#f8fafc", fontFamily: "'Inter',sans-serif" }}>
            ...
            <main style={{ flex: 1, display: "flex", flexDirection: "column", width: "100%", margin: "0 auto", padding: "32px 30px" }}>
                ...
                <DataTable
                    tableId="certificate_template_master"
                    title="Certificate Template Master"
                    data={templates}
                    columns={columns}
                    loading={loading}
                    searchPlaceholder="Search certificate templates..."
                    actionButton={
                        hasPermission("certificate_template_master", "write") ? (
                            <button
