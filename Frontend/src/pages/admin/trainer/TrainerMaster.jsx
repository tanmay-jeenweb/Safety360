...
// ─── Modal Form Component for Add & Edit Trainer ───────────────────────────────
function TrainerFormModal({ isOpen, onClose, onSave, editingRow, saving }) {
    const [trainerName, setTrainerName] = useState("");
    const [trainerEmail, setTrainerEmail] = useState("");

    useEffect(() => {
        if (editingRow) {
            setTrainerName(editingRow.trainer_name || "");
            setTrainerEmail(editingRow.trainer_email || "");
        } else {
            setTrainerName("");
            setTrainerEmail("");
        }
    ...
    const handleSubmit = (e) => {
        ...
        if (!trainerEmail.trim()) {
            toast.error("Trainer email is required");
            return;
        }
        onSave(trainerName.trim(), trainerEmail.trim());
    };
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    <div>
                        <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: "#fff" }}>
                            {editingRow ? "Edit Trainer" : "Add New Trainer"}
                        </h2>
                        <p style={{ margin: "4px 0 0", fontSize: 13, color: "#d9e2ec" }}>
                            {editingRow ? "Update trainer details below" : "Fill in the details to add a new trainer"}
                        </p>
                    </div>
                    <button
                        type="button"
                ...
                <form onSubmit={handleSubmit}>
                    <div style={{ padding: "24px" }}>
                        ...
                        <div style={{ marginBottom: 16 }}>
                            <label style={{
                                ...
                                Trainer Name <span style={{ color: "#e11d48" }}>*</span>
                            </label>
                            <input
                                type="text"
                                value={trainerName}
                        ...
                        <div style={{ marginBottom: 16 }}>
                            <label style={{
                                ...
                                Trainer Email <span style={{ color: "#e11d48" }}>*</span>
                            </label>
                            <input
                                type="email"
                                value={trainerEmail}
                    ...
                    <div style={{
                        ...
                        <button
                            type="submit"
                            disabled={saving || !trainerName.trim() || !trainerEmail.trim()}
                            style={{
                        ...
                        >
                            {saving ? "Saving…" : editingRow ? "Save Changes" : "Create Trainer"}
                        </button>
                    </div>
                </form>
            </div>
...
// ─── Main Trainer Master Page Component ─────────────────────────────────────────
export default function TrainerMaster() {
    ...
    const handleSave = async (trainerName, trainerEmail) => {
        setSaving(true);
        try {
            if (editingRow) {
                await updateTrainer(editingRow.id, { trainerName, trainerEmail });
                toast.success("Trainer updated successfully");
            } else {
                await createTrainer({ trainerName, trainerEmail });
            ...
            toast.error(err?.response?.data?.message || "Unable to save trainer.");
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            toast.error(err?.response?.data?.message || "Unable to delete trainer.");
        } finally {
            setSaving(false);
        }
    };
    ...
    const columns = useMemo(() => {
        const cols = [
            { key: "id", label: "ID", minWidth: "70px" },
            {
                key: "trainer_name",
                label: "Trainer Name",
                render: (row) => <span style={{ fontWeight: 700, color: "#1e293b" }}>{row.trainer_name}</span>
            },
            {
                key: "trainer_email",
                label: "Trainer Email",
        ...
        const canUpdate = hasPermission("trainer_master", "update");
        const canDelete = hasPermission("trainer_master", "delete");
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: "100vh", background: "#f8fafc", fontFamily: "'Inter',sans-serif" }}>
            ...
            <main style={{ flex: 1, display: "flex", flexDirection: "column", width: "100%", margin: "0 auto", padding: "32px 30px" }}>
                ...
                <DataTable
                    tableId="trainer_master"
                    title="Trainer Master"
                    data={trainers}
                    columns={columns}
                    loading={loading}
                    searchPlaceholder="Search trainers or emails..."
                    actionButton={
                        hasPermission("trainer_master", "write") ? (
                            <button
